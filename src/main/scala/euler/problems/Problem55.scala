...
/**
  ...
  *
  **/
object Problem55 extends Problem with App {

  private def isPalindrome(n: BigInt): Boolean = isPalindrome(n.toString)

  private def isPalindrome(s: String): Boolean = {
    s.length < 2 || (s.head == s.last && isPalindrome(s.substring(1, s.length - 1)))
  }

  def isLychrel(n: BigInt): Boolean = isLychrel(n, 50)

  private def isLychrel(n: BigInt, it: Int): Boolean = {
    val dup = n + BigInt(n.toString.reverse)
    it == 0 || (!isPalindrome(dup) && isLychrel(dup, it - 1))
  }

  override def solution(): Any = (BigInt(1) until BigInt(10000)).count(isLychrel)
